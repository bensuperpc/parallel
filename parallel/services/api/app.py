...
def upload():
    ...
    if compression_level < 0 or compression_level > 9:
        logger.warning("Invalid compression level: {}", compression_level)
    ...
    if preset < 0 or preset > 13:
        logger.warning("Invalid preset: {}", preset)
        return jsonify({"error": "Invalid preset"}), 400

    crf = int(request.args.get("crf", os.environ.get("DEFAULT_CRF", 2)))
    if crf < 0 or crf > 63:
        logger.warning("Invalid CRF: {}", crf)
...
if __name__ == '__main__':
    #logger.add("logs/api.log", rotation="1 MB", level="INFO", format="{time} {level} {message}", backtrace=True, diagnose=True)
