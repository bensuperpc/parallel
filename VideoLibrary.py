...
class VideoLibrary:
    def makefile_command(self, cmd : str):
        cmd = ["make", cmd]
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise Exception(f"Failed to makefile command: {result.stderr}")
        return result.stdout

    def upload_video(self, file_path, upload_endpoint):
        """Upload video file to the API.
        Like: curl -F "file=@video.mp4" "http://localhost:5000/upload?apikey=secret123"
        """
    ...
    def download_video(self, s3_output_key, download_endpoint, output_path):
        """Download video file from the API.
        Like: curl -o video_encoded.mp4 "http://localhost:5000/download?s3_output_key=output/encoded_video.mp4&apikey=secret123"
        """
